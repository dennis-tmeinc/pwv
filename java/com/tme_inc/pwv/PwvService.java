package com.tme_inc.pwv;

import android.app.Service;
import android.content.Intent;
import android.content.IntentFilter;
import android.content.SharedPreferences;
import android.net.LocalServerSocket;
import android.net.LocalSocket;
import android.os.BatteryManager;
import android.os.Handler;
import android.os.HandlerThread;
import android.os.IBinder;
import android.os.Looper;
import android.os.Message;
import android.widget.Toast;

import java.io.IOException;
import java.io.InputStream;
import java.io.OutputStream;
import java.net.InetSocketAddress;
import java.net.ServerSocket;
import java.net.Socket;
import java.net.SocketAddress;
import java.net.SocketException;
import java.util.HashSet;
import java.util.Iterator;

/**
 * Created by dennis on 09/06/15.
 */
public class PwvService extends Service {

    // virtual server support
    protected class vServer {
        public String targetDid ;
        public String targetPort ;

        public boolean running ;

        protected Thread mThread;
        protected ServerSocket server ;

        public vServer(String did, String port) {
            targetDid = did ;
            targetPort = port ;
            try {
                server = new ServerSocket(0);
                server.setSoTimeout(1800000);           // time out in half an hour
                running = true ;
            } catch (IOException e) {
                server = null ;
                running = false ;
            }
            mThread = new Thread(new Runnable() {
                @Override
                public void run() {
                    while ( running && server!=null && !server.isClosed() ) {
                        try {
                            Socket s = server.accept();
                            adConn nconn = new adConn(s);
                            synchronized (connSet) {
                                Iterator<adConn> it = connSet.iterator();
                                while (it.hasNext()) {
                                    adConn conn = it.next();
                                    if (conn.phase == 2 && conn.did.compareTo(targetDid)==0 ) {
                                        // request reversed connection
                                        nconn.phase = 3 ;
                                        connSet.add(nconn);
                                        conn.sendLine(String.format("connect %s * %s * 0\n", nconn.did, targetPort));
                                        break;
                                    }
                                }
                            }
                            if( nconn.phase == 3 ) {
                                nconn.start();
                            }
                            else {
                                nconn.close();
                                break ;
                            }

                        } catch (IOException e) {
                            running = false ;
                        }
                    }
                    running = false ;
                    try {
                        if( server !=null )
                            server.close();
                    } catch (IOException e) {}

                }
            });
            mThread.start();
        }

        void close() {
            running = false ;
            if( server!=null ) {
                try {
                    server.close();
                } catch (IOException e) {
                    e.printStackTrace();
                }
                server = null;
            }
            mThread.interrupt();
        }

        int port(){
            if( server!=null && !server.isClosed() ) {
                return server.getLocalPort();
            }
            return 0;
        }

    }

    HashSet <vServer> vServerSet = new HashSet<vServer>();

    // adb reverse connection
    protected class adConn {
        public int phase;      // 0: close, 1: init cmd, 2: unit wait, 3: connecting, 4: connected
        public Thread mThread;

        adConn target;         // target connection
        String did;
        String unitname;

        protected PwvSocket sock;
        boolean xoffflag ;

        adConn(Socket s) {
            sock = new PwvSocket(s);
            phase = 1;     // init
            xoffflag = false ;
            //did = Integer.toString(s.getPort());
            did = s.getRemoteSocketAddress().toString() ;
            mThread = null ;
        }

        public void start() {
            mThread = new Thread(new Runnable() {
                @Override
                public void run() {
                    while (phase > 0 && sock.isConnected()) {
                        if (phase <= 2) {
                            do_cmd();
                        } else if (phase == 3) {        // connecting, wait to be connected or closed
                            try {
                                Thread.sleep(10000);
                            }catch (InterruptedException e) {}
                            if( phase == 3 ) {          // not been connected in 10s
                                break;
                            }
                        } else if (phase == 4) {        // connected
                            if( xoffflag ) {
                                try {
                                    Thread.sleep(10000);
                                }catch (InterruptedException e) {}
                            }
                            else {
                                do_tunneling();
                            }
                        } else {
                            break;
                        }
                    }
                    close();
                    phase = 0;
                }
            });
            mThread.start();
        }

        public void close() {

            // clean up peers
            if( phase==2 ) {
                synchronized (connSet) {
                    Iterator<adConn> it = connSet.iterator();
                    while (it.hasNext()) {
                        adConn conn = it.next();
                        if (conn.phase != 2 ) {
                            conn.closepeer(this);
                        }
                    }
                }
            }
            else if( target!=null ) {
                target.closepeer(this);
                target = null ;
            }

            if ( sock.isConnected() ) {
                sock.close();
            }

            phase = 0;

            // interrupt the thread if possible
            if( mThread!=null && mThread.isAlive() ) {
                mThread.interrupt();
            }

        }

        public void closepeer( adConn peer ) {
            if (phase == 4 && target == peer) {
                target = null ;
                close();
            }
            else if (phase == 2) {
                sendLine(String.format("close %s\n", peer.did));
            }
        }

        public int sendfrom(byte[] data, int dsize, adConn from ) {
            synchronized (this) {
                if (phase == 4 && target == from)
                    return sock.send(data, dsize);
                else if (phase == 2) {
                    sendLine(String.format("mdata %s %d\n", from.did, dsize));
                    return sock.send(data, dsize);
                }
            }
            return 0;
        }

        public int sendLine(String line) {
            synchronized (this) {
                return sock.sendLine(line);
            }
        }

        public void xoff() {
            xoffflag = true ;
        }

        public void xon() {
            if(xoffflag) {
                xoffflag = false;
                mThread.interrupt();
            }
        }

        protected void do_cmd() {
            String l = sock.recvLine();
            String[] fields = l.split("\\s+");
            if (fields.length > 0) {

                // support command list:
                //    list:        list	sessionid	*|hostname  ( request a list of PW unit )
                //    remote:      remote sessionid idx port  ( request remote connection )
                //    unit:        unit name uuid   		( PW unit register )
                //    vserver:     vserver idx port         ( request virtual server to remote **** to be support in future )
                //    connected:   connected id             ( reverse connection from PW )
                //    close:       close id					( reverse connection failure )
                //    p:           ping                     ( null ping, ignor it)

                if (fields[0].compareTo("list") == 0) {
                    sendLine("rlist\n");
                    synchronized (connSet) {
                        Iterator<adConn> it = connSet.iterator();
                        while (it.hasNext()) {
                            adConn conn = it.next();
                            if (conn.phase == 2) {
                                sendLine(String.format("%s %s\n", conn.did, conn.unitname));
                            }
                        }
                    }
                    sendLine("\n");
                }else if (fields[0].compareTo("session") == 0) {
                    // cmd: session user key accesskey
                    if (fields.length > 1 && fields[1].compareTo("usb") != 0) {
                        sendLine("Error\n");
                    }
                    else {
                        sendLine("ok usb\n");     // session is not used on usb connection
                    }
                }else if (fields[0].compareTo("validate") == 0) {
                    // cmd: session user key accesskey
                    sendLine( "ok usb\n" );     // session is not used on usb connection
                } else if (fields[0].compareTo("remote") == 0 ) {
                    // cmd:  remote sessionid did port
                    if( fields.length>=4 ) {
                        synchronized (connSet) {
                            Iterator<adConn> it = connSet.iterator();
                            while (it.hasNext()) {
                                adConn conn = it.next();
                                if (conn.phase == 2 && conn.did.compareTo(fields[2])==0 ) {
                                    // request reversed connection
                                    conn.sendLine(String.format("connect %s * %s * 0\n", did, fields[3]));
                                    phase = 3 ;     // connecting
                                    return ;
                                }
                            }
                        }
                    }
                    sendLine("Error\n");

                } else if (fields[0].compareTo("mremote") == 0) {
                    // cmd:  remote sessionid did port
                    if( fields.length>=4 ) {
                        synchronized (connSet) {
                            Iterator<adConn> it = connSet.iterator();
                            while (it.hasNext()) {
                                adConn conn = it.next();
                                if (conn.phase == 2 && conn.did.compareTo(fields[2])==0 ) {
                                    // request reversed connection
                                    conn.sendLine(String.format("mconn %s * %s * 0\n", did, fields[3]));
                                    phase = 3 ;     // connecting
                                    return ;
                                }
                            }
                        }
                    }
                    sendLine("Error\n");

                } else if (fields[0].compareTo("unit") == 0) {
                    if (fields.length >= 3) {
                        synchronized (connSet) {
                            Iterator<adConn> it = connSet.iterator();
                            while (it.hasNext()) {
                                adConn conn = it.next();
                                if( conn == this )
                                    continue ;
                                if (conn.phase == 2 ) {
                                    conn.close();
                                    it.remove();
                                }
                            }
                        }
                        phase = 2;
                        unitname = fields[1];
                        did = fields[2];
                    }
                } else if (fields[0].compareTo("vserver") == 0) {
                    // cmd: vserver <sessionid> <deviceid> <port>
                    if( fields.length >= 4 ) {
                        // look for existing vserver
                        vServer vs = null ;
                        synchronized (vServerSet ) {
                            Iterator<vServer> it = vServerSet.iterator();
                            while (it.hasNext()) {
                                vs = it.next();
                                if( vs.targetDid.compareTo(fields[2])==0 && vs.targetPort.compareTo(fields[3])==0 ) {
                                    break ;
                                }
                                vs = null ;
                            }
                        }
                        if( vs == null ) {
                            vs = new vServer(fields[2], fields[3]);
                            synchronized (vServerSet ) {
                                vServerSet.add(vs);
                            }
                        }
                        sendLine(String.format("ok * %d\n", vs.port()) );
                    }
                    else {
                        sendLine("Error\n");
                    }

                } else if (fields[0].compareTo("connected") == 0 && fields.length>1 ) {
                    // cmd: connected id
                    // find target
                    synchronized (connSet) {
                        Iterator<adConn> it = connSet.iterator();
                        while (it.hasNext()) {
                            adConn conn = it.next();
                            if (conn.phase == 3 && conn.did.compareTo(fields[1]) == 0) {  // connecting
                                if( phase!=2 ) {
                                    target = conn;
                                    phase = 4;             // connected
                                }
                                conn.target = this;
                                conn.phase = 4;                 // connected
                                conn.mThread.interrupt();       // to wait up target thread
                                return;
                            }
                        }
                    }
                    // id not found!
                    if( phase==2 ) {
                        // response with closed connect
                        sendLine(String.format("close %s\n", fields[1]));
                    }
                } else if (fields[0].compareTo("close") == 0 ) {
                    if( fields.length>1 ) {
                        synchronized (connSet) {
                            Iterator<adConn> it = connSet.iterator();
                            while (it.hasNext()) {
                                adConn conn = it.next();
                                if (conn.did.compareTo(fields[1]) == 0) {  // connecting
                                    conn.target = null;
                                    if (conn.phase == 3) {
                                        conn.phase = 0;
                                        conn.mThread.interrupt();          // wakeup target conn
                                    }
                                    conn.phase = 0;
                                    break;
                                }
                            }
                        }
                    }
                    if( phase != 2 ) {
                        phase = 0 ;
                    }
                } else if (fields[0].compareTo("mdata") == 0) {
                    // multi use connection data
                    if( phase == 2 && fields.length>2 ) {
                        int si = Integer.parseInt(fields[2]);
                        if (si > 0 && si < 65536) {
                            byte[] buffer = new byte[si];
                            int r = sock.recv(buffer, 0, si);
                            if (r > 0) {
                                adConn f = null;
                                synchronized (connSet) {
                                    Iterator<adConn> it = connSet.iterator();
                                    while (it.hasNext()) {
                                        adConn conn = it.next();
                                        if (conn.did.compareTo(fields[1]) == 0 && conn.target == this) {
                                            f = conn;
                                            break;
                                        }
                                    }
                                }
                                if (f != null) {
                                    f.sendfrom(buffer, r, this);
                                }
                            }
                        }
                    }
                } else if (fields[0].compareTo("xon") == 0) {
                    if( fields.length>1 ) {
                        synchronized (connSet) {
                            Iterator<adConn> it = connSet.iterator();
                            while (it.hasNext()) {
                                adConn conn = it.next();
                                if (conn.did.compareTo(fields[1]) == 0 && conn.target == this) {
                                    conn.xon();
                                    break;
                                }
                            }
                        }
                    }
                } else if (fields[0].compareTo("xoff") == 0 && fields.length>1 ) {
                    // multi use connection xoff
                    if( fields.length>1 ) {
                        synchronized (connSet) {
                            Iterator<adConn> it = connSet.iterator();
                            while (it.hasNext()) {
                                adConn conn = it.next();
                                if (conn.did.compareTo(fields[1]) == 0 && conn.target == this) {
                                    conn.xoff();
                                    break;
                                }
                            }
                        }
                    }
                } else if (fields[0].compareTo("p") == 0) {
                    sendLine("e\n");
                } else if (fields[0].compareTo("hole") == 0 && fields.length>2 ) {
                    try {
                        Socket s = new Socket();
                        s.setReuseAddress(true);
                        s.bind(new InetSocketAddress(15601));
                        try {
                            s.connect(new InetSocketAddress(fields[1], Integer.valueOf(fields[2])), 50);
                        }
                        catch (IOException e) {
                            e.printStackTrace();
                        }
                        s.close();

                        ServerSocket ss = new ServerSocket(15601) ;
                        ss.setSoTimeout(600);
                        try {
                            s = ss.accept();
                            s.close();
                        }
                        catch (IOException e){}
                        ss.close();

                    } catch (SocketException e) {
                        e.printStackTrace();
                    } catch (IOException e) {
                        e.printStackTrace();
                    }

                } else if (fields[0].compareTo("kill-service") == 0) {      // debugging
                    stopSelf();
                }else {
                    sendLine("Unknown cmd\n");
                }
            }
        }

        // tunnelling data
        protected void do_tunneling() {
            if (sock.isConnected() && target != null ) {
                byte[] buffer = new byte[8192];
                int r = sock.recv1(buffer, 0, buffer.length);
                if (r > 0) {
                    if( target != null ) {
                        if( target.sendfrom(buffer, r, this)>0 )
                            return ;        // success
                    }
                }
            }
            // tunnel closed or target closed
            close();
        }
    }

    HashSet <adConn> connSet = new HashSet<adConn>();

    // clean up closed sockets
    protected void cleanupSockets() {

        synchronized (connSet) {
            Iterator<adConn> it = connSet.iterator();
            while (it.hasNext()) {
                adConn conn = it.next();
                if(conn.phase <= 0 ) {       // clean finished connection
                    conn.close();
                    it.remove();
                }
            }
        }

        synchronized (vServerSet) {
            Iterator<vServer> it = vServerSet.iterator();
            while (it.hasNext()) {
                vServer vs = it.next();
                if(!vs.running) {
                    vs.close();
                    it.remove();
                }
            }
        }
    }

    // main server for PwvService
    protected class PwvServer {
        protected Thread mThread;
        protected ServerSocket server ;

        public PwvServer() {

            SharedPreferences prefs = getSharedPreferences("pwv", 0);
            int sPort = prefs.getInt("loginPort", 15600);

            try {
                server = new ServerSocket(sPort);
                server.setSoTimeout(1800000);
            } catch (IOException e) {
                server = null ;
            }

            mThread = new Thread(new Runnable() {
                @Override
                public void run() {
                    while ( server!=null && !server.isClosed() ) {
                        try {
                            Socket s = server.accept();
                            if( s!= null ) {
                                adConn conn = new adConn(s);
                                synchronized (connSet) {
                                    connSet.add(conn);
                                }
                                conn.start();
                            }
                            cleanupSockets();
                        } catch (IOException e) {
                            cleanupSockets();
                            if (connSet.isEmpty()) {
                                break;
                            }
                        }
                    }
                    try {
                        if( server != null && !server.isClosed() )
                            server.close();
                    } catch (IOException e) {}

                    stopSelf();

                }
            });
        }

        void start() {
            if( !mThread.isAlive() ) {
                mThread.start();
            }
        }

        void close() {
            try {
                if( server != null && !server.isClosed() )
                    server.close();
            } catch (IOException e) {}
            server = null;
            if( mThread.isAlive() ) {
                mThread.interrupt();
            }

            synchronized (connSet) {
                Iterator<adConn> it = connSet.iterator();
                while (it.hasNext()) {
                    it.next().close();
                    it.remove();
                }
            }

            synchronized (vServerSet) {
                Iterator<vServer> it = vServerSet.iterator();
                while (it.hasNext()) {
                    it.next().close();
                    it.remove();
                }
            }

        }

    }

    PwvServer mServer ;

    @Override
    public void onCreate() {
        mServer = new PwvServer();
    }

    @Override
    public int onStartCommand(Intent intent, int flags, int startId) {
        mServer.start();
        return START_STICKY;
    }

    @Override
    public IBinder onBind(Intent intent) {
        // We don't provide binding, so return null
        return null;
    }

    @Override
    public void onDestroy() {
        mServer.close();
    }
}